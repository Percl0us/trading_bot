...
try:
    ...
    SECRET_KEY = st.secrets["BINANCE_TESTNET_SECRET_KEY"]
    BASE_URL = st.secrets.get("BINANCE_FUTURES_BASE_URL", "https://testnet.binancefuture.com")
except Exception:
    st.error("API keys not found in secrets. Please configure them in the Streamlit Cloud dashboard.")
    st.stop()

if not API_KEY or not SECRET_KEY:
    st.error("API keys are empty. Please check your secrets configuration.")
    st.stop()
...
with st.form("order_form"):
    ...
    with col2:
        ...
        quantity = st.number_input("Quantity", min_value=0.0001, value=0.001, step=0.0001, format="%.4f")

    price = None
    if order_type == "LIMIT":
        price = st.number_input("Price (USDT)", min_value=0.0, value=77000.0, step=100.0, format="%.2f")
